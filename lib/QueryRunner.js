...
export class QueryRunner {
    ...
    addQuery(...args) {
        return new Promise(resolve => {
            this.stack.push(Object.assign(new Query(...args), {resolve}))
    ...
    async executeQuery(query) {
        const serverResponse = await fetch(`${QueryRunner.basePath}/${query.method}?` +
            encodeToSearchString(this.access_token ? {...query.opts, access_token: this.access_token} : query.opts))
        const jsonResponse = await serverResponse.json()
        if (jsonResponse) {
            const {error, ...response} = jsonResponse
            if (!error)
                return response
            ...
            console.warn('bad server response', serverResponse)
